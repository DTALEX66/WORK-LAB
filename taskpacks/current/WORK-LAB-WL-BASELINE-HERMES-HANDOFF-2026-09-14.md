...
| 项 | 值 |
|---|---|
| 提交 | `763da6809acf93fee7d77c2a24f40b1734242b2c` |
| 父提交 | `c1c6a16454c06f4b90acf7e7913cc7dc62be3802` |
| 树对象 | `7198fed19843beb7d235856ce2e19cafeddad57d` |
| 规模 | 39 文件，+3158 / −77 |
| 候选指纹（提交前，24 modified + 15 untracked） | `755b40aaf74cf14186657b10f63983e077960fc7625f0e1d8edc7b2fa02fb5af` |
| 推送前扫描 | 对**全部 39 个**待提交文件按**内容形态**扫描 → 无凭据材料、无禁止文件类型、无绝对用户路径 |
| 双端一致性 | 本地 HEAD、`origin/r4-recovery-exec`、云端 API 读回的 commit 与 tree **三者相同**；本地 ahead/behind 均为 **0**；工作树 `clean` |

> 绑定关系说明：门禁证据绑定于**提交前的工作树**（候选指纹）。提交后工作树干净，故**提交 SHA 与树对象 SHA 取代指纹**作为权威标识。
> 本文件本身即该提交的一部分；**本提交之后**的提交 SHA 不在此列，属预期。

### 10.1 推送授权与范围声明

* 本次提交/推送依据**用户明确指令**（"上传云端仓库，确保双端仓库一致"）执行；此前全程为 **0 提交 0 推送**。
* **仓库为 PUBLIC**：上述 39 个文件的内容已对公众可见。
