...
Default enablement for a new candidate MCP still requires all existing Workflow MCP Stack gates; the existing Context7 portable baseline is the documented official-package-name/runtime-resolver exception:

1. Native Hermes tools cannot cover the use case.
2. Candidate version and license/source are pinned; the Context7 portable baseline remains unpinned at runtime.
