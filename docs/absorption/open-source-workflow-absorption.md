...
|---|---|---|
| `@upstash/context7-mcp` | 已吸收（默认） | 已通过 Hermes bundled Node wrapper 启用，用于实时拉取库文档；portable runtime 只声明官方包名，由 resolver 选择版本；保留旧 Node v16 失败记录仅作历史背景。 |
...
1. Context7 已默认启用；portable runtime 不固定版本，通过 `hermes-npx` 使用 Hermes bundled Node；candidate audit 如需复现历史供应链结果，另行记录 pinned provenance：
