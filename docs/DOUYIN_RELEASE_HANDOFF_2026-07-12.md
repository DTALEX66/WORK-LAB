...
- 接管后的第一轮是正常基线，玩家必须选择“判为正常”。
- 异常事件出现后，Canvas 明确提供“判为正常 / 报告异常”双选按钮，而不是系统自动替玩家判断。
- 正确判定记录统计并轻度奖励；误判和 6–7 秒超时都会扣稳定度并提高异常压力。
- 上报异常只完成识别，事件造成的现场状态仍保留，玩家还要使用控制台操作完成处置。
- 真实生成的 `game.js` VM 冒烟会推进到首个异常并验证双选按钮出现。

### 音频与本地偏好

- click / anomaly / result 三类音效均为仓库内本地 WAV，不依赖网络或第三方素材。
