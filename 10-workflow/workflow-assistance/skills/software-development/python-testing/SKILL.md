...
## `uv.lock` drift: phantom dependency names and never-locked new deps
...
## Dependency-declaration audits: imports alone don't prove a dep is used — or unused
...
## S-V-O regex extractors: passive-voice be-verbs get swallowed into the subject
...
## Full-suite warnings: probe with `-W error::DeprecationWarning` before deciding they matter
...
## Registry-driven compliance assertions: derive the blocked set from the source of truth
...
## Hard-coded schedule constants in a lifecycle INSERT = the feature is a stub
...
## CLI scripts that import library helpers: verify the function name exists first
...
## CI ruff scope often EXCLUDES tests/ — fix only your own violations
...
## Optional network stage in a pipeline: opt-in only, never in defaults
...
## Windows: SQLite temp files lock `TemporaryDirectory` cleanup (WinError 32)
...
## Editing test files: `patch` old_string must be UNIQUE
...
## Blind `sed -i` across a whole file when introducing a helper closure
...
## "Pre-existing failures" can be a missing dependency group, not code/network
...
### `uv pip install` may target a DIFFERENT venv than `uv run` uses
...
## `uv run ... pytest` → "uv trampoline failed to canonicalize script path": use `python -m pytest`
...
## Amended PR branch after CI already passed: force-push, then wait for a NEW green run
...
## CI test job fails at the toolchain-SETUP step → infrastructure, rerun, don't debug codeWhen a GitHub Actions `test` job fails at an early install step (e.g.
...
## A persistent `pytest.skip("...not produced in this environment")` can mask a real stage bug
...
## `--run-network`-style flags: register options before skipif references them
...
## Multi-provider aggregators: fake each provider's REAL response shape
...
### Cheap zero-coverage sweep before feature work
...
### The sweep generalizes to other packages — and to FUNCTION-level gaps
...
## `testpaths` can silently exclude an entire test package — and inclusion exposes DB pollution
...
## Function-local imports change the monkeypatch target
...
## Module-TOP imports: patch the CONSUMING module, not the source
...
### Module-level singleton INSTANCES: patch the attribute, and reset the init flag
...
## conftest TMPDIR redirection breaks path-scanning tests (fake file tree instead)
...
## Vendored ONNX models often ALREADY output probabilities — verify before softmax
...
## Windows: convention-gate CRLF findings can be checkout artifacts, not real issues
